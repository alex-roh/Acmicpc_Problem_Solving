...
int main(int argc, char** argv) {
	...
	rep(i, 0, V){
		...
		while(connectedVertex != -1){
			...
			int distance;
			ci(distance);
			graph[vertex].pb(mp(connectedVertex, distance));
			graph[connectedVertex].pb(mp(vertex, distance));
			
			ci(connectedVertex);
		}	
	}
	
	// 시작 정점에서 다른 모든 정점으로의 거리를 업데이트함
	visited[startVertex] = true;
	updateDistance(startVertex, 0);
	
	// 시작 정점에서 가장 먼 정점 farthestVertex를 구함 
